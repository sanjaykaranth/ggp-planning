...

{
  ...
  while ( TRUE ) {
    ...
    */
	...
	action_name(first->op,aname);
	
	
	if((strncmp(aname, "GOAL",4)==0))
	  {

	    int goal_val=atoi(aname+4);

	    /*printf("behold: %d \n", goal_val);*/

	    if(goal_val>0 && !is_goal_achieved(goal_val))
	      {
		extract_plan(first);
	
		set_goal_achieved(goal_val);

		printf("\nPlan for Goal value:%d",goal_val);
 
		print_sub_plan(gcurrent_plan);

		if(goal_val==100) /*|| current_time()>10) break;*/
		  {
		    break; /*keep searching till 100 point goal*/
		  }
	      }
	  }
      }
...

{
  ...
  h = get_reward_heuristic(S);/*get_1P( S, &ggoal_state );*/
...

{
  ...
  gnum_plans_ops[gcurrent_plan] = 0;
  for ( j = num_ops - 1; j > -1; j-- ) {

    gplans_ops[gcurrent_plan][gnum_plans_ops[gcurrent_plan]] = ops[j];
    gnum_plans_ops[gcurrent_plan]++;
...

{
	...
	if ( in_dest[gaxdels[i]] ) {
	    continue;
	}
	dest->F[dest->num_F++] = gaxdels[i];
	in_dest[gaxdels[i]] = TRUE;
    }
