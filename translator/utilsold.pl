...
my_unify(T1,T2,Constraints):-!,
        ...
        spliced_lists(T1Args,T2Args,Spliced),
        equality_from_bindings(Spliced,Constraints).
...
parse_assignments( [[E1,E2] | Rest], [Assign | Assigns], Nonassigns):-
        value_assignment([E1, E2], Assign), parse_assignments(Rest,
                                                              Assigns,
                                                              Nonassigns).
parse_assignments( [[E1,E2] | Rest], Assigns, [[E1,E2] | Nonassigns]):-
        \+value_assignment([E1, E2], Assign), parse_assignments(Rest,Assigns,Nonassigns).

value_assignment([E1,E2], equal(E1,E2)):-
        var(E1),ground(E2).
...
equal_groups( Varpairs, Groups):-
        split_list(Varpairs, Vars1,Vars2),
        list_to_set(Vars2, Set2),
        var_groups(Set2,Varpairs,Groups).
